...
- ارسال و دریافت فایل
- پیام‌رسان آنلاین (چت) با **ساعت به وقت تهران** و **گزارش خوانده شدن پیام با تیک دوگانه** (مشابه تلگرام)
- سیستم امضای الکترونیک اسناد
- **امضای فایل‌های PDF چندصفحه‌ای روی همه صفحات** — امکان جابه‌جایی بین صفحات سند و کشیدن و رها کردن (Drag & Drop) امضا روی هر صفحه دلخواه؛ سند امضاشده با همه صفحات و کیفیت اصلی ذخیره می‌شود
...
KAVENEGAR_API_KEY=کلید_API_کاوهنگار
```

> اگر `KAVENEGAR_API_KEY` در فایل .env تنظیم نشود، سامانه از کلید پیش‌فرض موجود در کد استفاده می‌کند.
...
## ۱۵. تغییرات نسخه جدید

### امضای PDF چندصفحه‌ای (همه صفحات)

در نسخه‌های قبلی فقط **صفحه اول** سند PDF امضا می‌شد و خروجی فقط یک تصویر از همان صفحه بود. در این نسخه:

- هنگام امضای یک PDF چندصفحه‌ای، نوار **جابه‌جایی صفحات** (قبلی / بعدی + شماره‌گذاری صفحات) نمایش داده می‌شود.
- با دکمه **«امضا در این صفحه»** امضا روی هر صفحه دلخواه فعال می‌شود و با **کشیدن و رها کردن (Drag & Drop)** در محل مناسب همان صفحه قرار می‌گیرد.
- هر صفحه مکان و اندازه امضای مستقل خودش را دارد؛ صفحاتی که امضا می‌شوند با نقطه سبز در شماره‌گذاری مشخص می‌شوند.
- پس از تأیید کد پیامکی، امضا **روی سند اصلی** اعمال می‌شود؛ یعنی فایل خروجی **همه صفحات PDF** را با همان کیفیت اولیه دارد (نه تصویر اسکن‌شده).
- گزارش امضا (در `uploads/documents/logs/`) شماره صفحات امضاشده را هم ثبت می‌کند.

### گزارش خوانده شدن پیام (تیک دوگانه) در پیام‌رسان

- پیام‌های ارسالی شما یک نشان وضعیت دارند:
  - **تک تیک** (سفید/کم‌رنگ): پیام تحویل داده شده اما هنوز خوانده نشده است.
  - **تیک دوگانه** (آبی روشن): گیرنده پیام را **خوانده** است — دقیقاً مشابه تلگرام.
- وضعیت خوانده شدن به‌صورت خودکار هر چند ثانیه به‌روزرسانی می‌شود.

### ساعت پیام‌رسان به وقت تهران

- نوع ستون‌های زمان در دیتابیس به `timestamptz` تغییر کرده است تا زمان دقیقاً و مستقل از تنظیمات منطقه زمانی سرور ذخیره شود.
- ساعت نمایش داده‌شده در پیام‌رسان همیشه به **وقت تهران** است؛ پیام‌های امروز فقط ساعت (مثل `۱۴:۰۵`) و پیام‌های قدیمی‌تر تاریخ + ساعت نمایش می‌دهند.

> **نکته برای اجرای روی سرور موجود:** بعد از جایگزینی کد جدید، یک بار `npx drizzle-kit push` را اجرا کنید تا نوع ستون‌های زمان در دیتابیس به‌روزرسانی شود.
...
## ۱۵. ابزارک‌های سامانه (نسخه جدید)

از منوی «ابزارک‌ها» در داشبورد به سه ابزار جدید دسترسی دارید:

### ۱. تبدیل فرمت فایل
مسیر: داشبورد → ابزارک‌ها → تبدیل فرمت فایل
- تبدیل تصاویر: PNG / JPG / WebP / BMP / GIF (به PNG، JPG یا WebP با تنظیم کیفیت)
- تبدیل PDF به تصویر (هر صفحه یک فایل PNG/JPG با کیفیت قابل تنظیم)
- استخراج متن از اسناد Word (docx / doc) به فایل TXT
- تبدیل فایل متنی (TXT / MD) به سند PDF (با پشتیبانی کامل فارسی)
- تبدیل CSV به JSON و برعکس
- تمام پردازش‌ها در مرورگر کاربر یا سرور داخلی انجام می‌شود و فایل جایی ارسال نمی‌گردد.

| فایل | توضیح |
|------|-------|
| `src/app/dashboard/tools/convert/page.tsx` | صفحه تبدیل فرمت |
| `src/app/api/convert/text/route.ts` | API استخراج متن Word |

### ۲. دستیار هوش مصنوعی
مسیر: داشبورد → ابزارک‌ها → دستیار هوش مصنوعی
- گفتگوی زنده (Streaming) با هوش مصنوعی رایگان
- مناسب نگارش نامه اداری، خلاصه‌سازی، ترجمه و پاسخ سوالات
- پیشنهادهای آماده، کپی پاسخ، تولید مجدد، توقف پاسخ، ذخیره تاریخچه در مرورگر
- به‌صورت پیش‌فرض از سرویس رایگان Pollinations (سازگار با OpenAI) استفاده می‌کند و نیازی به کلید ندارد.

تنظیم سرویس دلخواه (مثلاً DeepSeek) در فایل `.env`:
```
...
| فایل | توضیح |
|------|-------|
| `src/app/dashboard/tools/ai/page.tsx` | صفحه گفتگو |
| `src/app/api/ai/chat/route.ts` | API پروکسی هوش مصنوعی (SSE) |

### ۳. تصویر ماهواره‌ای
مسیر: داشبورد → ابزارک‌ها → تصویر ماهواره‌ای
- نمایش تصاویر ماهواره‌ای گوگل (هیبرید/خالص)، Esri و نقشه خیابان داخل خود سامانه
- جستجو با نام مکان (فارسی) یا مختصات اعشاری و درجه/دقیقه/ثانیه (DMS)
- محاسبه متراژ (مساحت به متر مربع، هکتار و کیلومتر مربع + محیط) با کلیک روی رئوس منطقه
- اندازه‌گیری فاصله مسیرها
- ثبت مختصات نقطه، کپی مختصات، نمایش مختصات زنده، موقعیت‌یاب GPS و حالت تمام‌صفحه
- خروجی GeoJSON برای استفاده در نرم‌افزارهای GIS — مناسب واحدهای شهرسازی و عمران

| فایل | توضیح |
|------|-------|
| `src/app/dashboard/tools/map/page.tsx` | صفحه نقشه ماهواره‌ای |
