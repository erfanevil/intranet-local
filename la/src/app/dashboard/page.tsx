...
import { FileCog, BotMessageSquare, Satellite, ArrowLeft, Wrench } from "lucide-react";
...
export default function DashboardPage() {
  ...
  // Add notification card only for users with permission
  const cards = user?.isAdmin || user?.canNotify
    ? [...baseCards, {
        title: "اطلاع‌رسانی پیامکی",
        desc: "مدیریت مخاطبین و ارسال پیامک",
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-xl bg-gradient-to-br from-slate-700 to-slate-900 text-white flex items-center justify-center shadow">
              <Wrench className="w-4 h-4" />
            </span>
            <h2 className="text-base font-bold text-slate-800">ابزارک‌های سامانه</h2>
          </div>
          <Link href="/dashboard/tools" className="text-xs font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1">
            مشاهده همه
          ...
          {[
            {
              ...
              desc: "تصویر، PDF، Word، CSV و JSON",
            ...
            },
            {
              ...
              desc: "گفتگو، نگارش نامه و خلاصه‌سازی",
            ...
            },
            {
              ...
              desc: "نقشه گوگل، متراژ و مختصات",
              color: "from-sky-500 to-emerald-500",
            },
          ].map((w) => (
            <Link key={w.href} href={w.href} className="group">
              <div className="relative overflow-hidden bg-white rounded-2xl border border-slate-200 p-4 flex items-center gap-3.5 hover:shadow-lg hover:-translate-y-0.5 hover:border-transparent transition-all">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${w.color} text-white flex items-center justify-center shadow group-hover:scale-110 transition-transform shrink-0`}>
                  <w.icon className="w-6 h-6" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-slate-800 text-sm">{w.title}</h3>
                  <p className="text-[11px] text-slate-500 mt-0.5 leading-5">{w.desc}</p>
                </div>
                <ArrowLeft className="w-4 h-4 text-blue-500 shrink-0 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
              </div>
            </Link>
          ))}
        </div>
