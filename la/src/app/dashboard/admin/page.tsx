...
interface User {
  ...
  avatar?: string; signature?: string; canSign: boolean; canNotify: boolean; isAdmin: boolean; createdAt: string;
}
...
export default function AdminPage() {
  ...
  const resetForm = () => {
    ...
    setIsAdmin(false); setCanSign(false); setCanNotify(false);
  ...
  const openEdit = (u: User) => {
    ...
    setPosition(u.position); setPassword(""); setPhone(u.phone || ""); setIsAdmin(u.isAdmin); setCanSign(u.canSign); setCanNotify(u.canNotify);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fd.append("isAdmin", String(isAdmin)); fd.append("canSign", String(canSign)); fd.append("canNotify", String(canNotify));
  ...
  const handleDelete = async (u: User) => {
    if (!confirm(`حذف "${u.displayName}"؟\n\nتمام پیام‌ها، فایل‌ها، چت‌ها و اسناد مرتبط نیز حذف خواهند شد.`)) return;
    setError("");
    try {
      const res = await authFetch(`/api/admin/users/${u.id}`, { method: "DELETE" });
      if (res.ok) { setSuccess("✅ کاربر با موفقیت حذف شد"); loadUsers(); }
      else { const data = await res.json(); setError(data.error || "خطا در حذف کاربر"); }
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {u.canNotify && <span className="bg-pink-100 text-pink-700 text-xs px-1.5 py-0.5 rounded-full">اطلاع‌رسانی</span>}
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="bg-slate-50 rounded-xl p-4 space-y-3">
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={canNotify} onChange={(e) => setCanNotify(e.target.checked)} className="w-4 h-4 text-pink-600 rounded border-slate-300" />
                  <div><p className="text-sm text-slate-700 font-medium">اطلاع‌رسانی پیامکی</p><p className="text-xs text-slate-400">دسترسی به ارسال پیامک گروهی</p></div>
                </label>
              </div>
