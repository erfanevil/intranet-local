...
// Chat bubble time (Tehran): only HH:mm for today's messages, date + time otherwise
export function formatChatTimeFA(dateStr: string): string {
  const d = new Date(dateStr);
  const timeFmt = new Intl.DateTimeFormat("fa-IR", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Tehran" });
  const dayFmt = new Intl.DateTimeFormat("fa-IR", { year: "numeric", month: "2-digit", day: "2-digit", timeZone: "Asia/Tehran" });
