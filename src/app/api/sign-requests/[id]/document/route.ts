...
export async function GET(
  ...
  // Build download name
  const originalBaseName = path.basename(req.documentOriginalName, path.extname(req.documentOriginalName));
  const originalExt = path.extname(req.documentOriginalName).toLowerCase();
  
  let downloadName: string;
  if (isSigned) {
    // Keep the same extension as the signed file (pdf stays pdf, png stays png)
    downloadName = `${originalBaseName}_امضا_شده${ext}`;
  } else {
    downloadName = req.documentOriginalName;
  }
