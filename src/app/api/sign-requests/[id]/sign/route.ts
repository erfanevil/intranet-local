...
async function loadPdfLib() {
  const vendorPath = path.join(process.cwd(), "src", "lib", "vendor", "pdf-lib.mjs");
  if (existsSync(vendorPath)) {
    const mod = await import(/* webpackIgnore: true */ vendorPath);
    return mod;
  }
...
export async function POST(
  ...
  try {
    ...
    if (originalExt === ".pdf" && existsSync(originalFilePath)) {
      // Try to create signed PDF
      let pdfCreated = false;
      const pdfLib = await loadPdfLib();

      if (pdfLib?.PDFDocument) {
        try {
          const originalPdfBytes = await readFile(originalFilePath);
          const pdfDoc = await pdfLib.PDFDocument.load(originalPdfBytes);
          const pngImage = await pdfDoc.embedPng(signedImageBuffer);
          ...
          if (pages.length > 0) {
            ...
            if (imgAspect > pageAspect) { dw = pw; dh = pw / imgAspect; }
            else { dh = ph; dw = ph * imgAspect; }
            page.drawImage(pngImage, { x: (pw - dw) / 2, y: (ph - dh) / 2, width: dw, height: dh });
          }
          ...
          console.error("[Sign] PDF embed error:", e);
        }
      }
