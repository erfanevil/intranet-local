...
export default function FilesPage() {
  ...
  const view = (f: F) => {
    ...
    const isExcel = lowerName.endsWith(".xls") || lowerName.endsWith(".xlsx") || f.mimeType.includes("excel") || f.mimeType.includes("sheet");
    
    if (isWord || isExcel) {
      // برای فایل‌های آفیس، دانلود کن تا با برنامه پیش‌فرض باز شود
      window.location.href = `/api/files/${f.id}/download?token=${getToken()}`;
    } else {
      // برای بقیه فایل‌ها در مرورگر باز کن
      window.open(`/api/files/${f.id}/view?token=${getToken()}`, "_blank");
    }
    markRead(f);
  };
