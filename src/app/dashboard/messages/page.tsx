...
interface ExtraAttachment {
  id: number; filename: string; originalName: string; size: number; mimeType: string;
}
interface Msg {
...
export default function MessagesPage() {
  ...
  const viewAttach = (m: Msg) => {
    ...
    if (isWord || isExcel) {
      // برای فایل‌های آفیس، دانلود کن تا با برنامه پیش‌فرض باز شود
      window.location.href = `/api/messages/${m.id}/attachment?token=${getToken()}`;
    } else {
      // برای بقیه فایل‌ها در مرورگر باز کن
      window.open(`/api/messages/${m.id}/attachment/view?token=${getToken()}`, "_blank");
    }
  };
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSel(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5 overflow-y-auto max-h-[calc(85vh-80px)]">
              ...
              {/* Attachments */}
              {(sel.attachmentOriginalName || (sel.extraAttachments && sel.extraAttachments.length > 0)) && (
                <div className="border-2 border-dashed border-slate-200 rounded-xl p-4">
                  <p className="text-sm font-medium text-slate-600 mb-3 flex items-center gap-1.5">
                    ...
                    پیوست‌ها ({(sel.attachmentOriginalName ? 1 : 0) + (sel.extraAttachments?.length || 0)})
                  </p>
                  <div className="space-y-2">
                    {sel.attachmentOriginalName && (
                      <div className="flex items-center justify-between bg-slate-50 p-3 rounded-lg">
                        <div className="flex items-center gap-3 min-w-0"><span className="text-xl">📄</span><div className="min-w-0"><p className="font-medium text-slate-800 text-sm truncate">{sel.attachmentOriginalName}</p><p className="text-xs text-slate-500">{fmtSize(sel.attachmentSize || 0)}</p></div></div>
                        <div className="flex gap-1.5 flex-shrink-0">
                          <button onClick={() => viewAttach(sel)} className="px-2.5 py-1 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg text-xs font-medium">مشاهده</button>
                          <button onClick={() => dlAttach(sel)} className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-medium">دانلود</button>
                        </div>
                      </div>
                    )}
                    {sel.extraAttachments?.map(att => (
                      <div key={att.id} className="flex items-center justify-between bg-slate-50 p-3 rounded-lg">
                        <div className="flex items-center gap-3 min-w-0"><span className="text-xl">📄</span><div className="min-w-0"><p className="font-medium text-slate-800 text-sm truncate">{att.originalName}</p><p className="text-xs text-slate-500">{fmtSize(att.size)}</p></div></div>
                        <button onClick={() => window.open(`/api/messages/attachments/${att.id}?token=${getToken()}`, "_blank")} className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-medium flex-shrink-0">دانلود</button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <button onClick={() => openMsg(m)} className="flex-1 flex items-start gap-3 text-right min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {((m.attachmentOriginalName ? 1 : 0) + (m.extraAttachments?.length || 0)) > 0 && (
                      <span className="flex items-center gap-0.5 text-slate-400 flex-shrink-0">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
                        {((m.attachmentOriginalName ? 1 : 0) + (m.extraAttachments?.length || 0)) > 1 && <span className="text-xs">{(m.attachmentOriginalName ? 1 : 0) + (m.extraAttachments?.length || 0)}</span>}
                      </span>
                    )}
                  </div>
