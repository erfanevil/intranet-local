...
export default function SignaturesPage() {
  ...
  // Render PDF page 1 to image using pdf.js from public folder
  const loadPdfAsImage = useCallback(async (docId: number) => {
    ...
    try {
      ...
      // Load pdf.js from public folder
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const canvas = document.createElement("canvas");
      const ctx = canvas.getContext("2d"); if (!ctx) throw new Error("خطا در ایجاد canvas");
  ...
  const verifySmsAndSign = async () => {
    if (!smsCode.trim() || !pendingBlob || !signingId) return; 
                  ...
                  <div className="flex flex-col items-center justify-center p-20">
                    {pdfLoading ? (
                      <>
                        ...
                        <span className="text-sm text-slate-500 mt-3">در حال تبدیل PDF...</span>
                      </>
                    ) : (
